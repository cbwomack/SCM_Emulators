...
def SolverBudykoSellers(const, grid, params, init, time):
  """
  Solve the Budyko-Sellers energy-balance model for a specified grid,
  forcing scenario, and diffusion configuration.

  Parameters
  ----------
  const : dict
    Physical constants (e.g., Earth radius, water density, heat capacity).
  grid : dict
    Grid definition including staggered latitudes and slab thicknesses.
  params : dict
    Model parameters and forcing settings.
  init : dict
    Initial temperature field and related state variables.
  time : dict
    Time-integration settings (time step, save interval, duration).

  Returns
  -------
  dict
    Simulation output containing temperature and flux time-series,
    diagnostic fields, and metadata.
  """
...
def Run_Budyko_Sellers(scen_flag=0, diff_flag=0, vert_diff_flag=0, xi=0, delta=0, n_boxes=3, noise_ts=None, spatial_flag=0, F0=3.7, int_yrs=250):
  """
  Integrate the Budyko-Sellers energy-balance model under a chosen
  forcing scenario and grid configuration.

  Parameters
  ----------
  scen_flag : int, optional
    Forcing scenario (0: abrupt 2xCO₂, 1: high, 2: mid, 3: overshoot,
    4: impulse).
  diff_flag : int, optional
    Horizontal diffusivity scheme (0: uncoupled, 1: coupled).
  vert_diff_flag : int, optional
    0 for a slab-ocean grid, 1 to include vertical diffusion.
  xi : float, optional
    Stochastic noise amplitude scaling.
  delta : float, optional
    Magnitude of impulse perturbation.
  n_boxes : int, optional
    Number of latitudinal boxes (3 or 5).
  noise_ts : ndarray, optional
    Prescribed noise time-series to ensure ensemble consistency.
  spatial_flag : int, optional
    Flag to modify spatial parameters (0: none, 1-3: custom pairs).
  F0 : float, optional
    Long-wave radiative forcing for the abrupt scenario [W m^2].
  int_yrs : int, optional
    Integration length in years.

  Returns
  -------
  dict
    Output from `SolverBudykoSellers`, including temperature time-series
    (`'T_ts'`), forcing, and diagnostic fields.
  """
  ...
  if vert_diff_flag == 0: # Regular grid
    if n_boxes == 5:
      ...
      grid['dz_slabs'] = np.array([1500, 5, 150, 5, 1500])  # Water slabs thickness [m]
    elif n_boxes == 3:
      grid['stag_lats'] = np.arange(-90, 91, 60)              # Vector of staggered latitudes [degrees]
      grid['dz_slabs'] = np.array([1500, 5, 150])  # Water slabs thickness [m]
  ...
  if spatial_flag != 0:
    grid['dz_slabs'] = np.array([5, 150, 1500])  # Water slabs thickness [m]
  ...
  time['dt'] = 3600 * 24 * 2                                                           # Time step [seconds]
